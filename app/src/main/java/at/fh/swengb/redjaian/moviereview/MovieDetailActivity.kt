...
class MovieDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (movieId == null) {
            ...
            director_content_detail.text = MovieRepository.movieById(movieId)?.director?.name.toString()
            actors_content_detail.text = MovieRepository.movieById(movieId)?.actors?.map { it.name }?.joinToString()
            ...
            avg_rating_bar_detail.rating = MovieRepository.movieById(movieId)?.ratingAverage()!!.toFloat()
            avg_rating_value_detail.text = MovieRepository.movieById(movieId)?.ratingAverage().toString()
